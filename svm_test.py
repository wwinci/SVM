...
if __name__ == '__main__':
    ...
    testDataList, testLabelList = loadData('svm_training_set.csv')

    # 开始测试
    print('start to test')
    predict = []
    test_data = testDataList[15000:17100]
    test_label = testLabelList[15000:17100]
    for i in range(len(test_data)):
         pre = svm.predict(test_data[i])
         pre = int(pre)
         print(pre,test_label[i])
         if pre == test_label[i]:
             print('True')
         else:
             print('False')
